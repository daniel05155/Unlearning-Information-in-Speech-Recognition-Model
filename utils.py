...
def write_config_log(logfile_path, model_type, epochs, batch_size, lr):
    ...
    with open(logfile_path, 'w') as f:
        ...
        # f.write(f'Use adam        = {use_adam}\n')
        f.write(f'Learning rate   = {lr}\n')
        # f.write(f'Scheduler step  = {milestones}\n')

def write_result_log(logfile_path, epoch, epoch_num, val_acc, val_loss, is_better):
    ...
    with open(logfile_path, 'a') as f:
        ...
        f.write(f'[{epoch + 1}/{epoch_num}]  | Val Acc: {val_acc:.5f} | Val Loss: {val_loss:.5f}')
        if is_better:
